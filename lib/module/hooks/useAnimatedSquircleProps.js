...
function cornerParamsW(cr, cs, preserveSmoothing, budget) {
  'worklet';
...
function assembleSvgPathW(w, h, tl, tr, br, bl) {
  'worklet';

  return `M ${fix4W(w - tr.p)} 0 ` + drawTRW(tr) + ` L ${fix4W(w)} ${fix4W(h - br.p)} ` + drawBRW(br) + ` L ${fix4W(bl.p)} ${fix4W(h)} ` + drawBLW(bl) + ` L 0 ${fix4W(tl.p)} ` + drawTLW(tl) + ' Z';
}
function distributeBudgetW(tlR, trR, brR, blR, w, h) {
  'worklet';
  ...
  const setB = (k, v) => {
    ...
    if (k === 'tl') tlB = v;else if (k === 'tr') trB = v;else if (k === 'br') brB = v;else blB = v;
  };
  const setR = (k, v) => {
    'worklet';

    if (k === 'tl') _tlR = v;else if (k === 'tr') _trR = v;else if (k === 'br') _brR = v;else _blR = v;
  };
  const keys = ['tl', 'tr', 'br', 'bl'].map(k => ({
    k,
    r: getR(k)
  })).sort((a, b) => b.r - a.r);
  for (const {
...
function computeSquirclePathW(width, height, cornerRadius, tlcr, trcr, brcr, blcr, cornerSmoothing) {
  'worklet';
  ...
  const {
    ...
    br,
    bl
  } = distributeBudgetW(tlR, trR, brR, blR, width, height);
  return assembleSvgPathW(width, height, cornerParamsW(tl.r, cornerSmoothing, false, tl.b), cornerParamsW(tr.r, cornerSmoothing, false, tr.b), cornerParamsW(br.r, cornerSmoothing, false, br.b), cornerParamsW(bl.r, cornerSmoothing, false, bl.b));
}
...
export function useAnimatedSquircleProps({
  ...
  const animatedPathProps = useAnimatedProps(() => {
    ...
    const inset = sw / 2;
    const d = computeSquirclePathW(width - sw, height - sw, cr > 0 ? Math.max(0, cr - inset) : cr, tlcr != null ? Math.max(0, tlcr - inset) : tlcr, trcr != null ? Math.max(0, trcr - inset) : trcr, brcr != null ? Math.max(0, brcr - inset) : brcr, blcr != null ? Math.max(0, blcr - inset) : blcr, cs);
    return {
      d,
      fill,
      stroke: sc,
...
//# sourceMappingURL=useAnimatedSquircleProps.js.map