...
function toRadiansW(degrees: number): number {
  'worklet'
  return (degrees * Math.PI) / 180
}

function fixedW(n: number): string {
  'worklet'
  return n.toFixed(4)
}
...
function getPathParamsForCornerW(
  cornerRadius: number,
  cornerSmoothing: number,
  preserveSmoothing: boolean,
  ...
  const arcSectionLength =
    Math.sin(toRadiansW(arcMeasure / 2)) * cornerRadius * Math.sqrt(2)
...
function drawTopRightW(params: CornerPathParams): string {
  'worklet'
  const { cornerRadius, a, b, c, d, p, arcSectionLength } = params
  if (cornerRadius) {
    return (
      `c ${fixedW(a)} 0 ${fixedW(a + b)} 0 ${fixedW(a + b + c)} ${fixedW(d)} ` +
      `a ${fixedW(cornerRadius)} ${fixedW(cornerRadius)} 0 0 1 ${fixedW(arcSectionLength)} ${fixedW(arcSectionLength)} ` +
      `c ${fixedW(d)} ${fixedW(c)} ${fixedW(d)} ${fixedW(b + c)} ${fixedW(d)} ${fixedW(a + b + c)}`
    )
  }
  return `l ${fixedW(p)} 0`
}

function drawBottomRightW(params: CornerPathParams): string {
  'worklet'
  const { cornerRadius, a, b, c, d, p, arcSectionLength } = params
  if (cornerRadius) {
    return (
      `c 0 ${fixedW(a)} 0 ${fixedW(a + b)} ${fixedW(-d)} ${fixedW(a + b + c)} ` +
      `a ${fixedW(cornerRadius)} ${fixedW(cornerRadius)} 0 0 1 -${fixedW(arcSectionLength)} ${fixedW(arcSectionLength)} ` +
      `c ${fixedW(-c)} ${fixedW(d)} ${fixedW(-(b + c))} ${fixedW(d)} ${fixedW(-(a + b + c))} ${fixedW(d)}`
    )
  }
  return `l 0 ${fixedW(p)}`
}

function drawBottomLeftW(params: CornerPathParams): string {
  'worklet'
  const { cornerRadius, a, b, c, d, p, arcSectionLength } = params
  if (cornerRadius) {
    return (
...
function drawTopLeftW(params: CornerPathParams): string {
  'worklet'
  const { cornerRadius, a, b, c, d, p, arcSectionLength } = params
  if (cornerRadius) {
    return (
...
function getSVGPathFromParamsW(
  width: number,
  height: number,
  tl: CornerPathParams,
  tr: CornerPathParams,
  ...
  'worklet'
  return (
    `M ${fixedW(width - tr.p)} 0 ` +
    drawTopRightW(tr) + ' ' +
    `L ${fixedW(width)} ${fixedW(height - br.p)} ` +
    drawBottomRightW(br) + ' ' +
    `L ${fixedW(bl.p)} ${fixedW(height)} ` +
    drawBottomLeftW(bl) + ' ' +
    `L 0 ${fixedW(tl.p)} ` +
    drawTopLeftW(tl) + ' Z'
  )
 ...
 */
function distributeAndNormalizeW(
  tlR: number,
  trR: number,
  brR: number,
  blR: number,
  ...
  const corners = [
    { key: 'tl' as const, radius: tlR },
    { key: 'tr' as const, radius: trR },
    { key: 'br' as const, radius: brR },
    { key: 'bl' as const, radius: blR },
  ].sort((a, b) => b.radius - a.radius)

  const getRadius = (key: 'tl' | 'tr' | 'br' | 'bl') => {
    'worklet'
  ...
  const getBudget = (key: 'tl' | 'tr' | 'br' | 'bl') => {
    'worklet'
  ...
  const setBudget = (key: 'tl' | 'tr' | 'br' | 'bl', value: number) => {
    'worklet'
    if (key === 'tl') tlBudget = value
    else if (key === 'tr') trBudget = value
    else if (key === 'br') brBudget = value
    else blBudget = value
  }
  const setRadius = (key: 'tl' | 'tr' | 'br' | 'bl', value: number) => {
    'worklet'
    if (key === 'tl') tlR = value
    else if (key === 'tr') trR = value
    else if (key === 'br') brR = value
    else blR = value
  }

  // adjacents: [corner, side-length]
  const adjacents: Record<'tl' | 'tr' | 'br' | 'bl', Array<['tl' | 'tr' | 'br' | 'bl', number]>> = {
  ...
  return {
    tl: { radius: tlR, roundingAndSmoothingBudget: tlBudget },
    tr: { radius: trR, roundingAndSmoothingBudget: trBudget },
    br: { radius: brR, roundingAndSmoothingBudget: brBudget },
    bl: { radius: blR, roundingAndSmoothingBudget: blBudget },
  }
}
...
export function getSvgPathWorklet({
  ...
  if (tlR === trR && trR === brR && brR === blR) {
    ...
    const params = getPathParamsForCornerW(
      clampedRadius,
      cornerSmoothing,
      preserveSmoothing,
      budget
  ...
  const tlParams = getPathParamsForCornerW(
    normalized.tl.radius,
    cornerSmoothing,
    preserveSmoothing,
    normalized.tl.roundingAndSmoothingBudget
  )
  const trParams = getPathParamsForCornerW(
    normalized.tr.radius,
    cornerSmoothing,
    preserveSmoothing,
    normalized.tr.roundingAndSmoothingBudget
  )
  const brParams = getPathParamsForCornerW(
    normalized.br.radius,
    cornerSmoothing,
    preserveSmoothing,
    normalized.br.roundingAndSmoothingBudget
  )
  const blParams = getPathParamsForCornerW(
    normalized.bl.radius,
    cornerSmoothing,
    preserveSmoothing,
    normalized.bl.roundingAndSmoothingBudget
  )
