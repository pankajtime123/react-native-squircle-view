...
import type { AnimatedSquircleViewProps, AnimatedSquircleParams } from '../types/animated'
 ...
 */
function resolveJS<T>(v: T | SharedValue<T>): T {
  if (v !== null && v !== undefined && typeof v === 'object' && 'value' in (v as object)) {
...
function snapshot(p: AnimatedSquircleParams): SquircleParams {
  return {
    ...
    fillColor:               resolveJS(p.fillColor ?? '#000000') as string,
    gradient:                p.gradient,
    strokeColor:             resolveJS(p.strokeColor ?? '#000000') as string,
    strokeWidth:             resolveJS(p.strokeWidth ?? 0),
  }
}

/** Shallow equality check on SquircleParams to avoid redundant re-renders. */
function paramsEqual(a: SquircleParams, b: SquircleParams): boolean {
  return (
    a.cornerRadius            === b.cornerRadius            &&
    a.topLeftCornerRadius     === b.topLeftCornerRadius     &&
    a.topRightCornerRadius    === b.topRightCornerRadius    &&
    a.bottomRightCornerRadius === b.bottomRightCornerRadius  &&
    a.bottomLeftCornerRadius  === b.bottomLeftCornerRadius  &&
 ...
 * re-renders occur.
 */
function useResolvedSquircleParams(animated: AnimatedSquircleParams): SquircleParams {
  const ref = useRef(animated)
  ref.current = animated

  const [params, setParams] = useState<SquircleParams>(() => snapshot(animated))
...
export function AnimatedSquircleView({
  ...
  children,
  ...viewProps
}: PropsWithChildren<AnimatedSquircleViewProps>): React.ReactElement {
  ...
  return (
    ...
    <Animated.View {...viewProps}>
      {/* Spread params directly — SquircleBackground destructures each field.
          Its internal <Rect> already uses absoluteFill to fill the parent. */}
      <SquircleBackground {...params} />
      {children}
    </Animated.View>
  )
}
